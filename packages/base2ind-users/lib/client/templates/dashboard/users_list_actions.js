...
Template.users_list_actions.helpers({
  isInvited: function()
  {
    return typeof this.appuser !== "undefined" ?  (this.appuser.hasOwnProperty('isInvited') ? this.appuser.isInvited : false) : false;
  },
...
Template.users_list_actions.events({
  'click .invite-link': function(e){
    e.preventDefault();
    Meteor.users.update(this._id, {$set: {"appuser.isInvited": true}});

  },
  'click .uninvite-link': function(e){
    e.preventDefault();
    Meteor.users.update(this._id, {$set: {"appuser.isInvited": false}});
  },
